...
namespace lua {
	NuklearBinding::NuklearBinding (LuaFile *LF) : LF(LF) {
		// Usertype for game core functions bindings should not be directly used in scripts
		auto nk = LF->state.new_usertype<NuklearBinding> ("__NuklearBindingType");
		/*---------- Export functions ----------*/
		#define NK_EXPORT_METHOD(field) {nk[#field] = &NuklearBinding::field;}
	...
	void NuklearBinding::init(nk_context *nkContext) {
		ctx = nkContext;
	}

	nk_context *NuklearBinding::ctx;

	/*---------- Auxiliary functions ----------*/

	inline struct nk_rect NuklearBinding::map2rect (float_map const &map) {
		return nk_rect (map.at ("x"), map.at ("y"), map.at ("w"), map.at ("h"));
	}


	inline NuklearBinding::float_map NuklearBinding::rect2map (struct nk_rect const &rect) {
		return {{"x", rect.x},
		        {"y", rect.y},
		        {"w", rect.w},
		        {"h", rect.h}};
	...
	bool NuklearBinding::begin (const char *title, lua_float_map bounds, int flags) {
		return (bool) nk_begin (ctx, title, map2rect (bounds), (nk_flags) flags);
	}


	NuklearBinding::lua_float_map NuklearBinding::window_get_bounds () {
		struct nk_rect nk_bounds = nk_window_get_bounds (ctx);
		return lua_float_map (rect2map (nk_bounds));
	}


	bool NuklearBinding::group_begin (const char *title, int flags) {
		return (bool) nk_group_begin (ctx, title, (nk_flags) flags);
	}


	void NuklearBinding::group_end () {
		nk_group_end (ctx);
	}


	void NuklearBinding::layout_row_dynamic (float height, int cols) {
		nk_layout_row_dynamic (ctx, height, cols);
	}


	void NuklearBinding::layout_row_static (float height, int item_width, int cols) {
		nk_layout_row_static (ctx, height, item_width, cols);
	}


	NuklearBinding::lua_float_map NuklearBinding::layout_space_bounds () {
		struct nk_rect nk_bounds = nk_layout_space_bounds (ctx);
		return lua_float_map (rect2map (nk_bounds));
	}


	NuklearBinding::lua_float_map NuklearBinding::widget_bounds () {
		struct nk_rect nk_bounds = nk_widget_bounds (ctx);
		return lua_float_map (rect2map (nk_bounds));
	}


	void NuklearBinding::label (const char *str, int flags) {
		nk_label (ctx, str, (nk_flags) flags);
	}


	void NuklearBinding::label_wrap (const char *str) {
		nk_label_wrap (ctx, str);
	}


	bool NuklearBinding::selectable_label (const char *str, int flags, bool selected) {
	...
	bool NuklearBinding::button_label (const char *str) {
		return (bool) nk_button_label (ctx, str);
	}


	bool NuklearBinding::check_label (const char *str, bool active) {
		return (bool) nk_check_label (ctx, str, active);
	}


	int NuklearBinding::edit_string (int flags, char* str, int max) {
		//return nk_edit_string_zero_terminated(ctx, flags, str, max, nk_filter_default);
		return 0;
	}


	bool NuklearBinding::popup_begin (int type, const char *title, int flags, lua_float_map bounds) {
		return (bool) nk_popup_begin (ctx, (nk_popup_type) type, title, (nk_flags) flags, map2rect (bounds));
	}


	void NuklearBinding::popup_close () {
		nk_popup_close (ctx);
	}


	void NuklearBinding::popup_end () {
		nk_popup_end (ctx);
	}


	int NuklearBinding::combo (sol::as_table_t<vector_str> items, int selected, int item_height, float width, float height) {
	...
	int NuklearBinding::combo_separator (const char *items, const char *separator, int selected, int count, int item_height,
	                     float width, float height) {
		return nk_combo_separator (ctx, items, separator[0], selected, count, item_height, nk_vec2 (width, height));
	}


	void NuklearBinding::tooltip (const char *str) {
		nk_tooltip (ctx, str);
	}


	bool NuklearBinding::input_is_mouse_hovering_rect (lua_float_map bounds) {
		struct nk_rect nk_bounds = map2rect (bounds);
		return (bool) nk_input_is_mouse_hovering_rect (&ctx->input, nk_bounds);
	}


	void NuklearBinding::finish () {
		nk_end (ctx);
	}
}
